...
int set_servo_angle(int angle) {
    ...
    int target_us = (int)(500 + (double)(angle + 90) / 180 * (2500 - 500));
    int target = (int)(target_us / 16);

    if (target < 31) target = 31;
    if (target > 156) target = 156;
...
void variable_register_changed(int value) {
    ...
    init_val = value;
        ...
        if(abs(preval - value) > 50){
            set_servo_angle(VR_value_to_servo_angle(value));
            if(value < 250){
                set_LED_separately(1, 0, 0, 0);
            }
            else if(value < 500){
                set_LED_separately(0, 1, 0, 0);
            }
            else if(value < 750){
                set_LED_separately(0, 0, 1, 0);
            }
            else{
                set_LED_separately(0, 0, 0, 1);
            }
            preval = value;
...
# 598 "3a.c"
    char str[100];
    set_servo_angle(VR_value_to_servo_angle(init_val));
    if(init_val < 250){
        set_LED_separately(1, 0, 0, 0);
    }
    else if(init_val < 500){
        set_LED_separately(0, 1, 0, 0);
    }
    else if(init_val < 750){
        set_LED_separately(0, 0, 1, 0);
    }
    else{
        set_LED_separately(0, 0, 0, 1);
