...
void variable_register_changed(int value) {
    ...
    if(mode == 1){
        if(abs(preval - value) > 50){
            ang = VR_value_to_servo_angle(value);
            clear_terminal();
            printf("%d", ang);
            preval = value;
        }
...
# 528 "1b.c"
    if(mode == 2){
        int mode2ang;
        sscanf(str, "%d", &mode2ang);
        if((-90 <= mode2ang) && (mode2ang <= 90)){
            set_servo_angle(mode2ang);
            mode = 0;
            printprompt = 1;
        }
        else{
    ...
    if(strcmp(str, "m1") == 0) {
        ...
        mode = 2;
        printf("Enter Target Angle[-90 to 90]\n");
    }
