...
void TMR0_Initialize(void){
    // Interrupt Every 0.1 sec (When Fosc = 4M)
    ...
    T0CONbits.T0PS = 0b010;     // Timer0 prescaler = 1:8 
    ...
    TMR0 = 53035;               //Start here to 65535(16bits)
...
void TMR1_Initialize(void){
    ...
    TMR1 = 53035; //Start here to 65535(16bits)
...
void TMR3_Initialize(void){
    // Interrupt Every 0.25 sec (When Fosc = 4M)
    ...
    T1CONbits.T1CKPS = 0b11;  // Timer3 prescaler = 1:8 
    T1CONbits.TMR1CS = 0b0;   // Timer3 clock source = internal clock (Fosc/4)
    
    TMR3 = 34285; //Start here to 65535(16bits)
...
void UART_Initialize(void){
    ...
    */
    ...
    // Setting Baud rate (when fosc = 4M)
...
void main() {
    ...
    while (1) {
        ...
        if(mode == 0){
            if(direction == 1){
                set_servo_angle(get_servo_angle() + 5);
                if(get_servo_angle() > 45){
                    direction = 0;
                }
            }
            else{
